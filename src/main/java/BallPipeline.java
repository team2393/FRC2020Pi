...
public class BallPipeline implements VisionPipeline
{
  ...
  // In principle looking for 'yellow' light, hue ~ 10
  // but biggest emphasis is on 'bright'.
  private final Scalar hsv_min = new Scalar( 20-10,  80.0, 150.0);
  private final Scalar hsv_max = new Scalar( 20+10, 255.0, 255.0);
  ...
  BallPipeline(final CvSource output, int width, int height) throws Exception
  {
    this.output = output;
    this.width = width;
    this.height = height;
  ...
  public void process(final Mat frame)
  {
    ...
    // Get largest contour, assuming that's also the closest ball
    ...
    final double aspect_min = SmartDashboard.getNumber("AspectMin", 0.8);
    final double aspect_max = SmartDashboard.getNumber("AspectMax", 1.2);

    final double fullness_min = SmartDashboard.getNumber("FullnessMin", 40.0);
